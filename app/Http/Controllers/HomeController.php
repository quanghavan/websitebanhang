...
class HomeController extends Controller
{
    ...
    {
        $slide = Slide::all();
        $newpr = San_pham::orderBy('id', 'DESC')->paginate(8, ['*'], 'newpr');
        $khuyenmai = San_pham::where('khuyen_mai', '<>', 0)->paginate(4, ['*'], 'khuyenmai');
        return view('frontend.page.content', compact('newpr', 'slide', 'khuyenmai'));
    }

    public function getLoaiSanPham($type)
    {
        $sp = San_pham::where('loai_spID', $type)->paginate(4, ['*'], 'sp');
        $km = San_pham::where('khuyen_mai', '<>', 0 )->where('Loai_spID',$type)->paginate(4,['*'], 'km');
